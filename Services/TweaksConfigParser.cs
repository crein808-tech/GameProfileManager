...
public static partial class TweaksConfigParser
{
        ...
        Dictionary<string, string>? pathOverrides = null)
    {
        ...
        {
            writer.WriteLine("[DLLPathOverrides]");
            foreach (var (key, value) in pathOverrides)
                writer.WriteLine($"{key} = {value}");
            writer.WriteLine();
        }
    }

    public static Dictionary<string, string> LoadPathOverrides(string iniPath)
    {
        ...
        foreach (var line in lines)
        {
            ...
            if (trimmed.StartsWith('[') && trimmed.EndsWith(']'))
            {
                inSection = trimmed.Equals("[DLLPathOverrides]", StringComparison.OrdinalIgnoreCase);
                continue;
            }
            if (!inSection || trimmed.StartsWith(';') || !trimmed.Contains('='))
                continue;
    ...
    public static void WritePathOverride(string iniPath, string dllName, string? path)
    {
        ...
        var sectionIdx = lines.FindIndex(l =>
            l.Trim().Equals("[DLLPathOverrides]", StringComparison.OrdinalIgnoreCase));

        if (sectionIdx < 0)
        {
            if (path is null) return;
            if (lines.Count > 0 && lines[^1] != "") lines.Add("");
            lines.Add("[DLLPathOverrides]");
            lines.Add($"{dllName} = {path}");
            File.WriteAllLines(iniPath, lines);
            return;
        }

        var keyPattern = new System.Text.RegularExpressions.Regex(
            $@"^\s*{System.Text.RegularExpressions.Regex.Escape(dllName)}\s*=",
        ...
        for (var i = sectionIdx + 1; i < lines.Count; i++)
        {
            ...
            if (keyPattern.IsMatch(lines[i])) { keyIdx = i; break; }
        }
        ...
        {
            lines[keyIdx] = $"{dllName} = {path}";
        }
        else
        {
            lines.Insert(sectionIdx + 1, $"{dllName} = {path}");
        }
